...
AFRAME.registerComponent('controller-updater', {
  init: function () {
    ...
    // --- Preset-pose button state ---
    this.rightAButton = false;
    this.rightBButton = false;

    // --- Preset-pose button combos ---
    // Hold RIGHT A+B together for 3s -> home both arms to the zero pose.
    // Hold LEFT X+Y together for 3s  -> park both arms to the hang-down rest pose.
    this.leftXButton = false;
    this.leftYButton = false;
    this.homeComboStart = null;
    this.parkComboStart = null;
    this.homeComboFired = false;
    this.parkComboFired = false;
    this.comboHoldMs = 3000;
    ...
    // Cloudflare uses a separate quick-tunnel hostname for the WS port.
    // start_tunnels.sh writes it to ws_config.js on every launch; keeping it
    // out of this source prevents stale cross-session connections.
    const wsTunnelHostname = window.TELEGRIP_WS_TUNNEL_HOSTNAME || "";
    const websocketUrl = serverHostname.endsWith("trycloudflare.com")
      ? `wss://${wsTunnelHostname}`
      : `wss://${serverHostname}:${websocketPort}`;
    ...
    try {
      ...
      this.websocket.onopen = (event) => {
        ...
        const skeletonEl = document.querySelector('[robot-skeleton]');
        const skeleton = skeletonEl && skeletonEl.components['robot-skeleton'];
        if (skeleton && skeleton._visible) {
          this.websocket.send(JSON.stringify({
            type: 'skeleton_subscription', enabled: true
          }));
        }
      };
      ...
      this.websocket.onmessage = (event) => {
        try {
          ...
          if (msg.type === 'robot_skeleton') {
            const skeletonEl = document.querySelector('[robot-skeleton]');
            if (skeletonEl && skeletonEl.components['robot-skeleton']) {
              skeletonEl.components['robot-skeleton'].updateFromMessage(msg);
            }
            return; // High-rate stream; don't log.
          }
          if (msg.type === 'vr_ui_config') {
            window.TELEGRIP_VR_UI_CONFIG = msg;
            this.setControllerAxesEnabled(
              msg.controller_axes_enabled_by_default);
            const skeletonEl = document.querySelector('[robot-skeleton]');
            const skeleton = skeletonEl &&
              skeletonEl.components['robot-skeleton'];
            if (skeleton) {
              skeleton.setEnabled(msg.shadow_enabled_by_default);
            }
            return;
          }
        } catch (e) { /* non-JSON server message, fall through to log */ }
    ...
    this.el.sceneEl.addEventListener('skeleton-visibility-changed', (evt) => {
      if (this.websocket && this.websocket.readyState === WebSocket.OPEN) {
        this.websocket.send(JSON.stringify({
          type: 'skeleton_subscription',
          enabled: Boolean(evt.detail.enabled)
        }));
      }
    });
    ...
    this.rightHand.addEventListener('abuttondown', () => { this.rightAButton = true; });
    this.rightHand.addEventListener('abuttonup', () => { this.rightAButton = false; });
    this.rightHand.addEventListener('bbuttondown', () => { this.rightBButton = true; });
    this.rightHand.addEventListener('bbuttonup', () => { this.rightBButton = false; });

    // --- Preset poses: Left X/Y buttons (X+Y together = park-pose combo) ---
    this.leftHand.addEventListener('xbuttondown', () => { this.leftXButton = true; });
    this.leftHand.addEventListener('xbuttonup', () => { this.leftXButton = false; });
    this.leftHand.addEventListener('ybuttondown', () => { this.leftYButton = true; });
    this.leftHand.addEventListener('ybuttonup', () => { this.leftYButton = false; });

    // --- Skeleton view toggle: press EITHER thumbstick (click) ---
    const toggleSkeleton = () => {
      const skeletonEl = document.querySelector('[robot-skeleton]');
      if (skeletonEl && skeletonEl.components['robot-skeleton']) {
        skeletonEl.components['robot-skeleton'].toggle();
      }
    };
    this.leftHand.addEventListener('thumbstickdown', toggleSkeleton);
    this.rightHand.addEventListener('thumbstickdown', toggleSkeleton);
  ...
  createAxisIndicators: function() {
    ...
    const leftStartIndex = this.leftHand.children.length;
    const rightStartIndex = this.rightHand.children.length;
    ...
    this.controllerAxisElements = [
      ...Array.from(this.leftHand.children).slice(leftStartIndex),
      ...Array.from(this.rightHand.children).slice(rightStartIndex)
    ];
    if (window.TELEGRIP_VR_UI_CONFIG) {
      this.setControllerAxesEnabled(
        window.TELEGRIP_VR_UI_CONFIG.controller_axes_enabled_by_default);
    }
    console.log('XYZ axis indicators created for both controllers (RGB for XYZ)');
  },

  setControllerAxesEnabled: function(enabled) {
    (this.controllerAxisElements || []).forEach((element) => {
      element.setAttribute('visible', Boolean(enabled));
    });
  },
  ...
  tick: function () {
    ...
    // Collect data from both controllers. Grip/trigger reflect the actual
    // button state even while the controller is briefly not tracked
    // (position stays null then): a tracking blip must NOT read as a grip
    // release on the server, or the arm halts and re-origins mid-motion.
    ...
    // --- Preset-pose combos: hold both buttons for comboHoldMs ---
    const comboNow = Date.now();
    if (this.rightAButton && this.rightBButton) {
        if (this.homeComboStart === null) this.homeComboStart = comboNow;
        if (!this.homeComboFired && comboNow - this.homeComboStart >= this.comboHoldMs) {
            this.homeComboFired = true;
            console.log('A+B held 3s -> homing arms to zero pose');
            if (this.websocket && this.websocket.readyState === WebSocket.OPEN) {
                this.websocket.send(JSON.stringify({pose: 'home'}));
            }
        }
    } else {
        this.homeComboStart = null;
    ...
    if (this.leftXButton && this.leftYButton) {
        if (this.parkComboStart === null) this.parkComboStart = comboNow;
        if (!this.parkComboFired && comboNow - this.parkComboStart >= this.comboHoldMs) {
            this.parkComboFired = true;
            console.log('X+Y held 3s -> parking arms to rest pose');
            if (this.websocket && this.websocket.readyState === WebSocket.OPEN) {
                this.websocket.send(JSON.stringify({pose: 'park'}));
            }
        }
    } else {
        this.parkComboStart = null;
    ...
    // Headset yaw (deg): the horizontal direction the operator is facing.
    // The server snapshots this when a grip engages, so "forward" is always
    // relative to the operator's gaze rather than the Quest room/floor frame.
    let headsetYawDeg = 0;
    const sceneCamera = this.el.sceneEl && this.el.sceneEl.camera;
    if (sceneCamera) {
        const gazeDir = new THREE.Vector3();
        sceneCamera.getWorldDirection(gazeDir);
        headsetYawDeg = THREE.MathUtils.radToDeg(Math.atan2(-gazeDir.x, -gazeDir.z));
    }

    // Send combined packet if WebSocket is open and at least one controller has valid data
    if (this.websocket && this.websocket.readyState === WebSocket.OPEN) {
        // (0,0,0) is a valid tracked-space coordinate after recentering.
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    if (scene) {
        ...
        if (scene.hasLoaded) {
            ...
            scene.setAttribute('robot-skeleton', '');

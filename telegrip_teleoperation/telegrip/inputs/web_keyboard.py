...
from ..config import TelegripConfig, POS_STEP, ANGLE_STEP
...
class WebKeyboardHandler(BaseInputProvider):
    ...
    def _set_keyboard_origin(self, arm: str):
        ...
        if self.robot_interface:
            try:
                ...
                # Orientation origin is captured by the control loop from the
                # actual TCP pose; the keyboard only sends relative roll/flex.
                arm_state["origin_wrist_roll"] = 0.0
                arm_state["origin_wrist_flex"] = 0.0
    ...
    def _send_gripper_goal(self, arm: str):
        ...
        No mode is attached: a gripper toggle must not change the control
        mode (mode=IDLE here used to silently drop position control for the
        whole arm whenever the gripper key was pressed mid-move).
        """
        arm_state = self.left_arm_state if arm == "left" else self.right_arm_state
        goal = ControlGoal(
            arm=arm,
